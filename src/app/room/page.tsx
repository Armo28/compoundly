...
export default function RoomPage() {
  ...
  // -------- headers (type-safe; never include undefined) --------
  ...
  const [respSaved, setRespSaved] = useState({
    total: '',
    life: '',
    year: '',
    kids: '',
  ...
  const strFromServer = (n: number | null | undefined) => {
    // show blanks for null/0 so the user doesn't fight a default "0"
    if (n == null || n === 0) return '';
    return String(n);
  };

  const dirtyRooms =
    tfsaStr !== roomsSaved.tfsaStr || rrspStr !== roomsSaved.rrspStr;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!r.ok) throw new Error('resp api unavailable');
        const j = await r.json();
  ...
  const saveResp = async () => {
    ...
    try {
      const payload: any = {
        ...
        children_covered: respFamily
          ? Math.max(1, Number(respStr.kids || '1'))
          : null,
      };
      ...
      const saved = {
        ...
        kids:
          payload.children_covered != null
            ? String(payload.children_covered)
            : '',
    ...
    return (
      <main className="max-w-6xl mx-auto p-4">
        <div className="rounded-xl border bg-white p-6">
          Sign in to edit room & RESP progress.
        </div>
      </main>
    );
  }
          ...
          <div className="grid grid-cols-1 sm:grid-cols-12 gap-3 items-end">
            <div className="sm:col-span-3 flex flex-col">
              ...
              <input
                ...
                value={respStr.total}
                onChange={(e) =>
                  setRespStr((s) => ({ ...s, total: e.target.value }))
                }
              />
            </div>
            ...
            <div className="sm:col-span-3 flex flex-col">
              ...
              <input
                ...
                value={respStr.life}
                onChange={(e) =>
                  setRespStr((s) => ({ ...s, life: e.target.value }))
                }
              />
            </div>
            ...
            <div className="sm:col-span-3 flex flex-col">
              ...
              <input
                ...
                value={respStr.year}
                onChange={(e) =>
                  setRespStr((s) => ({ ...s, year: e.target.value }))
                }
              />
            </div>
              ...
              {respFamily && (
                <>
                  ...
                  <input
                    ...
                    value={respStr.kids}
                    onChange={(e) =>
                      setRespStr((s) => ({ ...s, kids: e.target.value }))
                    }
                  />
                </>
              )}
