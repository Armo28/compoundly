...
export async function GET() {
  ...
  const { data, error } = await supa.from('children').select('*').eq('user_id', user.id).order('birth_year');
...
export async function POST(req: NextRequest) {
  ...
  const { data, error } = await supa.from('children')
    .insert({ user_id: user.id, name, birth_year: Number(birth_year) })
    .select('*').single();
